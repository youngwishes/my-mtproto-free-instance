...
Основной `playbook.yml` разворачивает приложение, удаляет прежний GeoIP
allowlist и затем включает limiter на хостах `mtproto_servers` последовательно
(`serial: 1`). Отдельный `telemt-syn-limit.yml` обновляет только limiter.
...
создаёт цепочку `TELEMT_SYN_LIMIT` и подключает её для входящих TCP SYN на порт
`443`. Если опциональный GeoIP allowlist включён отдельно, limiter размещается
после него; без GeoIP он становится первым правилом. Systemd unit идемпотентно
пересоздаёт правила при старте и удаляет все переходы в собственную цепочку и
саму цепочку при остановке. Ansible проверяет фактические правила и
перезапускает unit, если firewall был очищен независимо от systemd.
