...
`telemt-geo-allow` разрешает новые подключения к опубликованному Docker-порту
Telemt `443/tcp` только из российских IPv4-сетей. IPv6 не входит в scope:
прокси его не поддерживает.
...
Список обновляется только при запуске `playbook.yml` или отдельного
`telemt-geo-allow.yml`. Фонового timer или cron нет.
